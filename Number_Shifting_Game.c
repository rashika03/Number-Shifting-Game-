...
void randNumGen(int arr[][4])
{
    ...
    srand(time(0)); // per execution random number different generate
    for (i = 0; i < 4; i++)
    {
        j = 0;
        while (j < 4)
...
int main()
{
    ...
    char arrowKey, opt;            // take user input from arrow key
    int randNum[4][4];             // random Number Generate in Matrix
    char userName[30];             // take UserName

    printf("Enter your name: ");
    fgets(userName, 30, stdin); // input userName

    randNumGen(randNum); // Fill Random Num in Matrix

    if (menu()) // First we show our menu if user press Y means continue or N means N. it all handle this if conditions
    {
        while (totalMoves--)
        {
            display(randNum, totalMoves, userName); // Display matrix, user name, and totalMoves
            arrowKey = getch();
            arrowKey = getch();
